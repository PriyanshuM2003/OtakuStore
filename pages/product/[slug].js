      ...
      <div className="container px-5 py-20 mx-auto">
        <div className="lg:w-4/5 mx-auto flex flex-wrap justify-center items-center">
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <h1 className="leading-relaxed text-gray-900 font-medium mt-1">{product.desc}</h1>
              ...
              {product.category === "T-Shirt" && (
                <div className="flex">
                  ...
                  {Object.keys(variants).map((colorOption) => (
                    <button
                      key={colorOption}
                      onClick={() => { refreshVariant(colorOption, selectedSize) }}
                      className={`border-2 ${selectedColor === colorOption ? "border-black" : "border-gray-400"} ml-1 rounded-full w-6 h-6 focus:outline-none`}
                      style={{ backgroundColor: colorOption }}
                    ></button>
                  ))}
                </div>
              )}
              <div className={product.category === "T-Shirt" ? "flex ml-6 items-center" : "flex my-4 items-center"}>
                <div className='absolute z-0'>
                  <span className="mr-3">Size</span>
                  <select
                    value={selectedSize}
                    onChange={(e) => { refreshVariant(selectedColor, e.target.value) }}
                  ...
                  >
                    {selectedColor &&
                      Object.keys(variants[selectedColor]).map((sizeOption) => (
            ...
            <div className="flex">
              {product.availableQty <= 0 && <span className="title-font font-semibold text-3xl text-purple-800">Out of stock!</span>}
              {product.availableQty > 0 && <span className="title-font font-medium text-4xl text-purple-800">₹{product.price}</span>}
              {product.availableQty > 0 && <button onClick={() => { buyNow(slug, 1, product.price, product.title, selectedSize, selectedColor, product.img) }} className="flex ml-4 text-white text-lg font-semibold bg-purple-800 border-0 py-2 px-6 focus:outline-none hover:text-yellow-400 rounded">Buy Now</button>}
              {product.availableQty > 0 && <button onClick={() => { addToCart(slug, 1, product.price, product.title, selectedSize, selectedColor, product.img) }} className="rounded-full w-11 h-11 bg-purple-800 p-0 border-0 inline-flex items-center justify-center text-gray-100 ml-4">
                <AiOutlineShoppingCart onClick={notify} className='text-3xl hover:text-yellow-400' />
              </button>}
            ...
            {product.availableQty > 0 && <div className="flex mt-8 md:mt-3 md:flex-nowrap flex-wrap justify-center items-center md:items-end md:justify-end">
              <div className="flex sm:w-32 w-40 sm:mr-4 mr-2">
                <input onChange={onChangePincode} type="text" id="Pincode" placeholder='Pincode' name="Pincode" className="w-full rounded border border-purple-900 focus:ring-2 focus:ring-purple-400 focus:border-purple-900 text-base outline-none text-gray-700 py-1 px-3 leading-8" />
              </div>
              <button onClick={checkServiceAvail} className="inline-flex text-white bg-purple-800 border-0 py-2 px-6 hover:text-yellow-400 font-semibold focus:outline-none rounded">Check</button>
            </div>}
            {product.availableQty > 0 && (!service && service != null) && <div className='flex justify-center items-center md:justify-end md:items-end text-red-600 text-sm mt-3'>Sorry! Delivery service is not available to this pincode</div>}
            {product.availableQty > 0 && (service && service != null) && <div className='flex justify-center items-center md:justify-end md:items-end text-green-600 text-sm mt-3'>Delivery service is available to this pincode</div>}
          </div>
        </div>
      </div>
    </section>
