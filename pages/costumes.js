...
const Costumes = ({ products }) => {
  ...
  return <>
    <Head>
      <title>Otaku Store- Costumes</title>
    ...
    <section className='min-h-screen text-gray-700 body-font'>
      <div className="bg-white mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="md:flex md:items-baseline justify-between inline-block border-b border-gray-200 pb-4 pt-20">
          <h1 className="text-4xl font-bold tracking-tight text-purple-900">Costumes</h1>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: 'Costume' })

  let costume = {}
  for (let item of products) {
    if (item.title in costume) {
      if (!costume[item.title].size.includes(item.size) && item.availableQty > 0) {
        costume[item.title].size.push(item.size)
      }
    } else {
      costume[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        costume[item.title].size = [item.size]
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(costume)) },
  }
}
