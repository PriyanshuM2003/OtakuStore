...
const handler = async (req, res) => {
  if (req.method == "POST") {
    if (!Object.keys(pincodes).includes(req.body.pincode)) {
      res.status(200).json({
        success: false,
        error: "Sorry! Delivery service is not available to this pincode",
        cartClear: false,
      });
      return;
    }
    ...
    for (let item in cart) {
      ...
      if (product.availableQty < cart[item].qty) {
        res.status(200).json({
          success: false,
          error:
            "Item is currently out of stock. Please try again sometime later!",
          cartClear: true,
        });
        return;
      }
      if (product.price != cart[item].price) {
        res.status(200).json({
          success: false,
          error: "The price of item in your cart has changed. Please try again",
          cartClear: true,
        });
        return;
      }
    ...
    if (
      ...
      res.status(200).json({
        success: false,
        error: "Please enter your valid pincode",
        cartClear: false,
      });
      return;
    }
