    ...
    try {
      ...
      if (Rres.id) {
        var options = {
          ...
          description: "data",
          image:
            "https://pbs.twimg.com/profile_images/1345466407237128192/3TNO2kqs_400x400.jpg",
            ...
            <div className="lg:w-4/5 w-full lg:pr-10 lg:py-6 mb-6 lg:mb-0">
              <div className="sideCart">
                ...
                {Object.keys(cart).length == 0 && (
                  <div className="flex text-2xl font-semibold justify-center items-center text-purple-800">
                    Your Cart is empty!
                  </div>
                )}
                          ...
                          >
                            ...
                            <td className="py-4 px-6 font-semibold text-purple-800 dark:text-white">
                              ...
                              <h1 className="flex text-gray-500 text-xs title-font font-medium">
                                Size:
                                <p className="text-purple-900 text-xs title-font font-medium ml-1">
                                  {cart[k].size}
                                </p>
                              </h1>
                ...
                <div className="flex mb-2">
                  <a className="flex-grow text-purple-800 font-semibold border-b-2 border-purple-500 py-2 text-lg px-1">
                    Details
                  </a>
                </div>
                {user && user.token ? (
                  <p className="leading-relaxed font-semibold">{name}</p>
                ) : (
                  ""
                )}
                {user && user.token ? (
                  <p className="leading-relaxed font-semibold">{phone}</p>
                ) : (
                  ""
                )}
                {user && user.token ? (
                  <p className="leading-relaxed font-semibold">{user.email}</p>
                ) : (
                  ""
                )}
                {user && user.token ? (
                  address.length !== 0 ? (
                    ...
                    <p className="leading-relaxed font-bold py-2 text-red-600">
                      To further proceed please click on User Icon and fill out
                      address details in My Accounct section!
                    </p>
                  )
                ) : (
                ...
                {user && user.token ? (
                  ...
                  <p className="leading-relaxed font-bold text-red-600">
                    Please Login/SignUp to continue your purchase!
                  </p>
                )}
                <div className="mt-2 border-b-2 border-purple-500"></div>
                <div className="flex mt-2 ">
                  {Object.keys(cart).length > 0 && (
                    <span className="title-font font-medium text-2xl text-purple-800">
                      Total: ₹{subTotal}
                    </span>
                  )}
                  {Object.keys(cart).length > 0 && address.length !== 0 && (
                    <button
                      onClick={makePayment}
                      className="flex ml-auto text-white font-semibold bg-purple-900 disabled:bg-slate-500 disabled:hover:text-white border-0 py-2 px-6 focus:outline-none hover:text-yellow-400 rounded"
                    >
                      Place Order
                    </button>
                  )}
