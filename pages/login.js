...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (response.success) {
      ...
      localStorage.setItem('myuser', JSON.stringify({ token, email, name, phone }))
      ...
      <div className="min-h-full flex items-center justify-center py-20 px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-md w-full space-y-8">
          <div>
            ...
            <h2 className="mt-6 text-center text-3xl font-extrabold text-purple-800">Login to your account</h2>
            <p className="mt-2 text-center text-sm text-gray-600">
              Or <br />
              <Link href={'/signup'} legacyBehavior>
                <h1 className="font-medium text-purple-900 hover:text-purple-700"> Sign-up</h1>
              </Link>
            </p>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm">
                <Link href={'/forgot'} legacyBehavior>
                  <h1 className="font-medium text-indigo-600 hover:text-indigo-500"> Forgot your password? </h1>
                </Link>
              </div>
            </div>
            <div>
