...
const AddImage = () => {
  ...
  const submitForm = async (e) => {
    e.preventDefault();
    try {
      const selectedCategories = Object.keys(form.category).filter(
        (key) => form.category[key]
      );
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/addimages`,
        {
          method: "POST",
          headers: {
    ...
    <CacheProvider value={cache}>
      <>
        <BaseCard title="Add Product">
          ...
          <br />
          <Button onClick={submitForm} scrollToTop variant="outlined" mt={2}>
            Upload
          </Button>
        </BaseCard>
      </>
